...
def preprocess_image(img: np.ndarray) -> np.ndarray:
    ...
    show_image(out, "gray", True, cmap="gray")

    out = cv2.GaussianBlur(out, (5, 5), 0)  # kernel size, sigma
    show_image(out, "blurred", True, cmap="gray")

    _, out = cv2.threshold(out, 0, L, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    show_image(out, "thresholded", True, cmap="gray")

    out = cv2.equalizeHist(out)
    show_image(out, "histogram equalised", True, cmap="gray")

    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    out = cv2.morphologyEx(out, cv2.MORPH_CLOSE, kernel)
    show_image(out, "morphed", True, cmap="gray")

    out = cv2.Canny(out, 30, 150)  # threshold1, threshold2
    show_image(out, "edge detected", True, cmap="gray")
